...
export default function LoginForm() {
  ...
  const bigCommercePasswordResetPage =
    bigCommerceURL + '/login.php?action=reset_password';
  const bigCommerceCreateAccountPage =
    bigCommerceURL + '/login.php?action=create_account';
  ...
  async function getRefreshToken(e) {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_WORDPRESS_URL}/wp-json/atlas-commerce-connector-bigcommerce/v1/login`,
        {
          method: 'POST',
          headers,
          body: JSON.stringify({
      ...
      if (data.status == 200) {
        ...
        setErrorMessage(
          'Username or password is invalid. Please try again or reset your password at the link below.'
        );
      }
    } catch (error) {
      console.log(error);
  ...
  async function getRedirectUrl() {
    ...
    try {
      const response = await fetch(
        `${process.env.NEXT_PUBLIC_WORDPRESS_URL}/wp-json/atlas-commerce-connector-bigcommerce/v1/bc-link`,
        {
          method: 'POST',
          headers: {
      ...
      <div>
        <div>
          ...
          <a
            ...
            href={bigCommercePasswordResetPage}
            target='_blank'
            rel='noreferrer'
          >
            Lost your password?
          ...
          <a
            className={styles['forgot-password']}
            href={bigCommerceCreateAccountPage}
            target='_blank'
            rel='noreferrer'
          >
            Create Account
